...
export function ClawbacksPage() {
  ...
  const { data: clawbackBatches = [], isLoading } = useQuery({
    queryKey: ['clawback-batches'],
    queryFn: async () => {
      const response = await api.get('/clawbacks/batches');
  ...
  const tableColumns = [
    ...
    { key: 'deductPeriod', label: 'Kỳ bị trừ', align: 'left' as const, minWidth: '120px' },
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedItems(clawbackBatches.map(b => b.id));
    } else {
      setSelectedItems([]);
    }
  };
      ...
      <div className="p-[16px]">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[8px] p-[16px]">
            ...
            <table className="w-full">
              <thead>
                <TableHeader
                  ...
                  showCheckbox={true}
                  allChecked={selectedItems.length === clawbackBatches.length && clawbackBatches.length > 0}
                  onCheckAll={handleSelectAll}
                />
              </thead>
              <tbody>
                {clawbackBatches.map((batch) => (
                  <tr key={batch.id} className="border-b border-[#e5e7eb] hover:bg-gray-50">
                    ...
                    <td className="px-[16px] py-[12px] text-[14px] text-[#111827]">
                      {batch.deductPeriod ? `${batch.deductPeriod.month}/${batch.deductPeriod.year}` : '-'}
                    </td>
                    <td className="px-[16px] py-[12px] text-[14px] text-[#111827] text-right">
          ...
          <TableFooter
            ...
            totalPages={Math.ceil(clawbackBatches.length / itemsPerPage)}
            itemsPerPage={itemsPerPage}
            totalItems={clawbackBatches.length}
